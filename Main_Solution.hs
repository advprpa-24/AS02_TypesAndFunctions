...
preorder :: Bin a -> [a]
preorder (Leaf a) = [a]
preorder (Fork l a r) =  a : preorder l ++ preorder r
...
inorder :: Bin a -> [a]
inorder (Leaf a) = [a]
inorder (Fork l a r) = inorder l ++ [a] ++ inorder r
...
postorder :: Bin a -> [a]
postorder (Leaf a) = [a]
postorder (Fork l a r) = postorder l ++ postorder r ++ [a]
...
-- names = map name -- eta reduced version
...
foldrLength :: [a] -> Int
foldrLength as = foldr (\_ len -> len + 1) 0 as
...
-- Use the function `foldr`:
foldrMap :: (a -> b) -> [a] -> [b]
foldrMap f as = foldr (\a acc -> f a : acc) [] as
